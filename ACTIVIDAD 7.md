# Actividad7_CA

- Samuel Esteban Pichillá Duartes 202505036
- Hasel Abigail Girón Urbina 202504693
- Josselyn Gabriela Mendoza Camargo 202500317
- Hennsen Armando Rodrigo Aldana Villalobos 202500286

| Estudiante       | Rol        | Responsabilidades                                                              |
|------------------|------------|-------------------------------------------------------------------------------|
| Josselyn Mendoza | Moderador  | Coordinar el equipo, facilitar la comunicación, asegurar que todos participen |
| Samuel Pichillá   | Timekeeper | Controlar tiempos, asegurar cumplimiento de deadlines, gestionar cronograma   |
| Hennsen Aldana y Hasel Abigail | Notetaker  | Documentar decisiones, registrar acuerdos, consolidar documento final         |

# Día 1 - Inicio del Proyecto

**[10:00 AM] Josselyn (Moderador):**  
"Buenos días equipo. Espero que se encuentren bien.  
Les escribo para dar inicio a nuestro Mini-Proyecto 1 sobre el Sistema de Gestión de Tareas Académicas.  

**Objetivo:** Desarrollar una propuesta básica para organizar tareas académicas.  

**Roles asignados:**  
- Yo seré el Moderador  
- Samuel será el Timekeeper  
- Hennsen y Hasel serán Notetaker  

Por favor, confirmen recepción y disponibilidad para comenzar."

---

**[10:15 AM] Samuel (Timekeeper):**  
"Buenos días, Josselyn. Confirmo mi rol como Timekeeper.  

**Cronograma propuesto:**  
- Día 1-2: Definición del problema y funcionalidades  
- Día 3-4: Roles del sistema y flujo de uso  
- Día 5: Consolidación del documento final  

¿Les parece bien? Necesito que me confirmen para establecer los deadlines específicos."

---

**[10:30 AM] Hennsen y Hasel (Notetaker):**  
"Buenos días a ambos. Confirmo mi participación como Notetaker.  

El cronograma me parece adecuado. Crearé un documento compartido en Google Docs para ir registrando nuestros avances y decisiones.  

**Link del documento**   

Por favor, accedan y comiencen a aportar sus ideas."

---

# Día 2 - Desarrollo de Funcionalidades

**[9:00 AM] Josselyn (Moderador):**  
"Buenos días equipo.  

**Agenda de hoy:**  
- Definir el problema a resolver (30 min)  
- Establecer funcionalidades principales (mínimo 4) (1 hora)  

Samuel, por favor monitorea los tiempos. Hennsen, por favor documenta las decisiones.  

Comencemos con la definición del problema. ¿Cuáles son sus propuestas?"

---

**[9:10 AM] Samuel (Timekeeper):**  
"Confirmo el inicio de la sesión. Tenemos 1.5 horas para completar estos dos puntos.  

**Time check:** 9:10 AM - Inicio  
**Deadline parcial:** 10:40 AM para tener definidos problema y funcionalidades"

---

**[9:15 AM] Hennsen Hennsen y Hasel (Notetaker):**  

**PROBLEMA DEFINIDO:**  
Los estudiantes universitarios enfrentan dificultades para:  
- Organizar múltiples tareas académicas con diferentes fechas de entrega  
- Priorizar actividades según urgencia e importancia  
- Visualizar el progreso de sus responsabilidades  
- Coordinar trabajos en equipo de manera efectiva  

**PROPUESTA DE FUNCIONALIDADES:**  
1. Registro de actividades con fecha de entrega  
2. Sistema de priorización (Alta/Media/Baja)  
3. Visualización de progreso mediante dashboard  
4. Notificaciones de vencimientos próximos  

¿Están de acuerdo? ¿Agregamos algo más?"

---

**[9:35 AM] Josselyn (Moderador):**  
"Excelente trabajo, Hennsen y Hasel.  

Propongo agregar una quinta funcionalidad:  
5. Asignación de tareas en trabajos colaborativos  

Esto permitiría que los estudiantes coordinen mejor sus proyectos grupales.  

Samuel, ¿cómo vamos de tiempo?"

---

**[9:40 AM] Samuel (Timekeeper):**  
"Actualización de tiempo:  
- Problema definido: COMPLETADO  
- Funcionalidades: En proceso (llevamos 25 min de 60 asignados)  
- Funcionalidad 5 aprobada  

**Status:** Vamos bien. Continuemos con la documentación de funcionalidades detalladas."

---

# Día 3 - EVENTO: PÉRDIDA DE CONEXIÓN

**[2:00 PM] Josselyn (Moderador):**  
"Buenas tardes. Iniciamos la sesión para definir roles del sistema y flujo de uso.  

Samuel, ¿estás disponible? Hennsen, por favor confirma.  
[Esperando 10 minutos...]"

---

**[2:15 PM] Josselyn (Moderador):**  
" **SITUACIÓN DE EMERGENCIA**   

Samuel no ha respondido y parece tener problemas de conexión.  

**PROCEDIMIENTO DE FALLBACK ACTIVADO:**  
- Intentaré contactar a Samuel por WhatsApp  
- Si no responde en 15 minutos, redistribuiremos sus tareas  
- Hennsen y yo cubriremos temporalmente sus responsabilidades  

**Redistribución temporal:**  
- Hennsen y Hasel: Asumirán control de tiempos además de documentación  
- Yo: Coordinaré ambas áreas hasta que Samuel se reincorpore  

Hennsen y Hasel, ¿están de acuerdo con este plan?"

---

**[2:20 PM] Hennsen y Hasel (Notetaker):**  
"Confirmo. Asumo temporalmente el rol de Timekeeper además de Notetaker.  

**Plan de acción:**  
- Continuaré documentando todo  
- Estableceré checkpoints de tiempo cada 30 minutos  
- Mantendré el cronograma original  

Mientras tanto, intentaré contactar a Samuel por correo electrónico."

---

**[2:25 PM] Josselyn (Moderador):**  
"Perfecto. Continuemos con la definición de roles del sistema.  

**ROLES DE USUARIO PROPUESTOS:**  
- Estudiante: Puede crear, editar y visualizar sus tareas  
- Profesor: Puede asignar tareas, establecer deadlines y ver progreso del curso  
- Administrador: Gestiona usuarios y configuración del sistema  

¿Alguna observación?"

---

**[2:45 PM] Hennsen y Hasel (Notetaker/Timekeeper temporal):**  
"**TIME CHECK:** 2:45 PM  

- Roles del sistema: COMPLETADO  
- Próximo punto: Flujo de uso (45 min restantes)  

Nota: Samuel acaba de escribir al grupo de WhatsApp. Tuvo una falla eléctrica. Estará de vuelta en 20 minutos.  

**Decisión:** Continuamos con el flujo de uso y Samuel se integra cuando pueda."

---

**[3:10 PM] Samuel (Timekeeper) - REINCORPORADO:**  
"¡Equipo! Disculpen la desconexión. Tuve una falla eléctrica inesperada.  

Ya estoy de vuelta. ¿En qué punto estamos?  

Gracias por activar el procedimiento de fallback y mantener el proyecto en marcha. Asumo nuevamente mis responsabilidades como Timekeeper."

---

**[3:12 PM] Josselyn (Moderador):**  
"¡Bienvenido de vuelta, Samuel!  

**Status actual:**  
- Problema definido  
- Funcionalidades establecidas (5)  
- Roles del sistema definidos (3)  
- Flujo de uso: En proceso  

Por favor, retoma el control del tiempo. Nos faltan definir los flujos de uso."

---

**[3:15 PM] Samuel (Timekeeper):**  
"Gracias por la actualización.  

**TIME CHECK:** 3:15 PM  
- Pendiente: Flujo de uso y consolidación  
- Tiempo restante hoy: 45 minutos  

**Propuesta de FLUJO DE USO:**  

**Para Estudiantes:**  
1. Login  
2. Dashboard  
3. Crear/Ver tarea  
4. Asignar prioridad  
5. Marcar como completada  

**Para Profesores:**  
1. Login  
2. Panel de curso  
3. Crear asignación  
4. Establecer deadline  
5. Ver progreso estudiantil  

¿Completamos esto hoy o lo finalizamos mañana?"

---

**[3:30 PM] Hennsen y Hasel (Notetaker):**  
"Excelente propuesta, Samuel.  

**DECISIÓN DEL EQUIPO:**  
Dejaremos el flujo de uso documentado como está y mañana temprano haremos la consolidación final del documento.  

**Tareas para mañana:**  
- Revisión final del documento  
- Agregar diagrama de flujo visual  
- Formatear según requisitos  
- Entrega final  

**Lección aprendida:** El procedimiento de fallback funcionó correctamente. Pudimos continuar sin perder tiempo significativo."

## ISSUES/TAREAS CREADAS

| Issue # | Título                                    | Asignado a | Estado          | Prioridad |
|---------|-------------------------------------------|------------|-----------------|----------|
| #001    | Definir problema a resolver               | Todos      | ✅ Completado    | Alta     |
| #002    | Establecer funcionalidades principales    | Josselyn   | ✅ Completado    | Alta     |
| #003    | Definir roles del sistema (usuarios)      | Samuel     | ✅ Completado    | Alta     |
| #004    | Describir flujo de uso                    | Hennsen y Hasel | ✅ Completado    | Alta     |
| #005    | Crear documento compartido                | Hennsen y Hasel | ✅ Completado    | Media    |
| #006    | Establecer cronograma                     | Samuel     | ✅ Completado    | Media    |
| #007    | Consolidar documento final                | Todos      | ❌ En progreso   | Alta     |
| #008    | Activar procedimiento de fallback         | Josselyn   | ✅ Completado    | Crítica  |
| #009    | Redistribuir tareas por desconexión       | Hennsen y Hasel | ✅ Completado    | Crítica  |

# PRESENTACIÓN FINAL

## Cómo se organizó el equipo

### Asignación clara de roles

- **Josselyn Mendoza (Moderador)**: Coordinó las sesiones, facilitó la comunicación, tomó decisiones cuando fue necesario y activó el procedimiento de fallback ante la desconexión
- **Samuel Pichillá (Timekeeper)**: Estableció cronograma, monitoreó tiempos en cada sesión, aseguró el cumplimiento de deadlines
- **Hennsen Aldana y Hasel Girón (Notetaker)**: Documentó todas las decisiones, consolidó el documento final, mantuvo registro de issues

### Distribución del trabajo

- Las 5 tareas principales se dividieron equitativamente
- Cada miembro tuvo responsabilidad primaria en al menos 1 tarea
- Revisión cruzada: todos revisaron el trabajo de los demás

### Herramientas utilizadas

- Chat grupal para comunicación sincrónica
- Documento compartido en Google Docs
- Sistema de issues para tracking de tareas

---

## Cómo gestionaron la comunicación

### Etiqueta profesional aplicada

- **Tono**: Formal pero cercano, respetuoso en todo momento
- **Claridad**: Mensajes estructurados con viñetas y negritas
- **Estructura**: Saludo + contexto + acción requerida + cierre

### Frecuencia de comunicación

- Check-ins diarios (mañana y tarde)
- Respuestas dentro de las 2 horas máximo
- Notificaciones inmediatas para urgencias

### Canales utilizados

- **Chat principal**: Decisiones y coordinación
- **Documento compartido**: Documentación formal
- **WhatsApp (backup)**: Contacto de emergencia

---

## Dificultades encontradas al trabajar de forma remota

### Problema de conexión (Día 3)

- **Situación**: Samuel perdió conexión por falla eléctrica
- **Impacto**: 20 minutos de retraso en la sesión
- **Solución**: Activación inmediata del procedimiento de fallback

### Coordinación de horarios

- **Situación**: Disponibilidad diferente en algunos horarios
- **Solución**: Establecimiento de bloques fijos de trabajo (9 AM - 12 PM)

### Comunicación asincrónica

- **Situación**: Demora en respuestas cuando no todos estaban conectados
- **Solución**: Uso de mensajes claros con acciones específicas y deadlines

---

## Estrategias utilizadas para resolver problemas

### Procedimiento de fallback documentado

- Protocolo claro para pérdida de conexión
- Redistribución inmediata de tareas
- Múltiples canales de contacto (chat, email, WhatsApp)

### Roles bien definidos

- Cada miembro conocía sus responsabilidades
- El moderador tenía autoridad para tomar decisiones rápidas
- El timekeeper aseguraba que los retrasos no afectaran el cronograma

### Documentación en tiempo real

- El notetaker registraba todo inmediatamente
- Evitó pérdida de información
- Permitió que Samuel se reincorporara rápidamente al estar documentado

### Comunicación proactiva

- Anticipación de problemas (Samuel avisó apenas pudo)
- Transparencia en el estado de las tareas
- Feedback constante entre miembros

---

## LECCIONES APRENDIDAS

1. **La planificación de contingencias es esencial**: El procedimiento de fallback funcionó perfectamente, permitiendo continuar sin pérdidas mayores.

2. **Los roles claros mejoran la eficiencia**: Cada miembro sabía exactamente qué hacer, evitando duplicación de esfuerzos.

3. **La documentación en tiempo real es crucial**: Permitió la reincorporación rápida de Samuel y mantuvo a todos alineados.

4. **La comunicación profesional previene malentendidos**: El uso de tono, claridad y estructura apropiados facilitó el trabajo remoto.

5. **La flexibilidad es clave**: La capacidad de adaptar roles temporalmente (Hennsen asumió Timekeeper + Notetaker) salvó la sesión.
